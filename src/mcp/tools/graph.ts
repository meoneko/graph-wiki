...
import { GetGraphStatsInput } from '../schemas/index.js';

export function registerGraphTools(): void {
  registerTool({
    name: 'graph_stats',
    description: 'Graph statistics filtered by trust mode',
    inputSchema: GetGraphStatsInput,
    handler: async (args) => {
      const input = GetGraphStatsInput.parse(args);
  ...
  registerTool({
    ...
    handler: async (args) => {
      ...
      if (result.status === 'POLICY_VIOLATION') return result;
